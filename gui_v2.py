...
# Loading image
user_image = ImageTk.PhotoImage(Image.open(relative_to_assets("User Photo.png")))
home_icon = ImageTk.PhotoImage(
    Image.open(relative_to_assets("home .png")).resize((15, 15), Image.ANTIALIAS)
)
prepare_icon = ImageTk.PhotoImage(
    Image.open(relative_to_assets("prepare.png")).resize((15, 15), Image.ANTIALIAS)
)
process_icon = ImageTk.PhotoImage(
    Image.open(relative_to_assets("process.png")).resize((15, 15), Image.ANTIALIAS)
)
review_icon = ImageTk.PhotoImage(
    Image.open(relative_to_assets("review.png")).resize((15, 15), Image.ANTIALIAS)
)
logout_icon = ImageTk.PhotoImage(
    Image.open(relative_to_assets("logout.png")).resize((15, 15), Image.ANTIALIAS)
)
standard_icon = ImageTk.PhotoImage(
    Image.open(relative_to_assets("standard.png")).resize((15, 15), Image.ANTIALIAS)
)
floating_icon = ImageTk.PhotoImage(
    Image.open(relative_to_assets("floating.png")).resize((15, 15), Image.ANTIALIAS)
)
new_temp_icon = ImageTk.PhotoImage(
    Image.open(relative_to_assets("new_temp.png")).resize((15, 15), Image.ANTIALIAS)
)
search_icon = ImageTk.PhotoImage(
    Image.open(relative_to_assets("search.png")).resize((15, 15), Image.ANTIALIAS)
)
upload_file_icon = ImageTk.PhotoImage(
    Image.open(relative_to_assets("upload_file.png")).resize((80, 80), Image.ANTIALIAS)
)
...
docs_icon = ImageTk.PhotoImage(
    Image.open(relative_to_assets("docs.png")).resize((50, 50), Image.ANTIALIAS)
)
export_icon = ImageTk.PhotoImage(
    Image.open(relative_to_assets("export.png")).resize((25, 25), Image.ANTIALIAS)
)
create_icon = ImageTk.PhotoImage(
    Image.open(relative_to_assets("create.png")).resize((25, 25), Image.ANTIALIAS)
)
import_icon = ImageTk.PhotoImage(
    Image.open(relative_to_assets("import.png")).resize((25, 25), Image.ANTIALIAS)
)
save_icon = ImageTk.PhotoImage(
    Image.open(relative_to_assets("save.png")).resize((25, 25), Image.ANTIALIAS)
)
...
def home():
    ...
    dashboard_frame = Frame(container_frame, width=600, height=300, bg="#C4C4C4")
...
def stand_new_template():
    ...
    Button(canvas, image=create_icon, bd=0, bg="white").place(relx=0.934, rely=0.7963)
    Button(canvas, image=import_icon, bd=0, bg="white").place(relx=0.934, rely=0.8704)
    Button(canvas, image=save_icon, bd=0, bg="white").place(relx=0.934, rely=0.9444)


def standard():
    clear_frame(container_frame)
    containt_frame = Frame(container_frame, width=800, height=540, bg="#F3F3FB")
    containt_frame.place(relx=0.05, rely=0.0555, relheight=0.9, relwidth=0.9091)
    templates_types = ["Recent Templates", "Save Templates", "Deleted Templetes"]
    variable = StringVar()
    variable.set(templates_types[1])
    templates_types_dropdown = OptionMenu(containt_frame, variable, *templates_types)
    templates_types_dropdown.place(relx=0, rely=0.1, relheight=0.07, relwidth=0.2)
    ...
    temp_list = [template_1, template_2, template_3, template_4, template_5, template_6]
    for i in range(6):
        l1 = Label(
            temp_list[i],
...
def process():
    ...
    Label(
        ...
        font=("Arials", 15, "bold"),
        bg="#F3F3FB",
    ).place(relx=0.00625, rely=0.00926)
    upload_frame_1 = Frame(containt_frame, width=350, height=250, bg="white")
    upload_frame_1.place(relx=0.00625, rely=0.0926, relheight=0.463, relwidth=0.4375)
    Label(upload_frame_1, image=upload_file_icon, bg="white").place(
        relx=0.4, rely=0.3, relheight=0.35
    )
    Label(upload_frame_1, text="Drag and Drop Files", bg="white", fg="#707C97").place(
        relx=0.37, rely=0.65, relheight=0.05
    )
    upload_frame_2 = Frame(containt_frame, width=350, height=250, bg="white")
    upload_frame_2.place(relx=0.50625, rely=0.0926, relheight=0.463, relwidth=0.4375)
    Label(upload_frame_2, image=upload_file_icon, bg="white").place(
        relx=0.4, rely=0.3, relheight=0.35
    )
    Label(upload_frame_2, text="Drag and Drop Files", bg="white", fg="#707C97").place(
        relx=0.37, rely=0.65, relheight=0.05
    )

    Label(
        containt_frame,
        text="Export Result as",
        font=("Arials", 12, "bold"),
        bg="#F3F3FB",
    ).place(relx=0.2, rely=0.65, relheight=0.1)
    Label(containt_frame, image=pdf_icon, bg="#F3F3FB").place(
        relx=0.375, rely=0.645, relheight=0.1
    )
    Label(containt_frame, image=docs_icon, bg="#F3F3FB").place(
        relx=0.45, rely=0.645, relheight=0.1
    )
    Label(containt_frame, image=xls_icon, bg="#F3F3FB").place(
        relx=0.525, rely=0.645, relheight=0.1
    )
    Button(
        containt_frame,
        text="   Export",
        ...
        fg="white",
        width=140,
    ).place(relx=0.35, rely=0.8, relheight=0.1, relwidth=0.2)
...
def review():
    ...
    table_container_frame = Frame(containt_frame, width=790, height=480, bg="white")
    table_container_frame.place(relx=0.05, rely=0.08, relheight=0.85, relwidth=0.9)
    table_frame = Frame(table_container_frame, bg="white")
    table_frame.place(relx=0, rely=0, relheight=0.95, relwidth=1)
    ...
    pt.show()
    cols = Entry(table_container_frame)
    cols.place(relx=0.01, rely=0.955, relheight=0.045, relwidth=0.15)
    Button(
        table_container_frame, text="Bar Chart", command=partial(bar_chart, df, cols)
    ).place(relx=0.2, rely=0.955, relheight=0.045, relwidth=0.1)
